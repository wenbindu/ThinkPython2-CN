...
1999年1月, 我正準備以Java教學生編程入門, 
課程已講過三次, 但我沮喪依舊. 
因為多人無法完成目標, 雖有優秀個體, 但整體效果令人難以接受.

依我觀察, 問題出在書本教案上, 
書籍厚重, 畫蛇添足, 細枝末節過多, 編程思慮不足.
因此學生總是陷於困境: 起步易, 行進難, 半途而廢.
學生學得囫圇吞棗, 老師教得步步回首.

此次開課前兩週, 我決定自己寫本書, 目標如下：
...
首版略顯粗略, 但是簡單有效. 
學生知其然, 並知其所以然, 我便可聚焦課堂於難點, 趣題和操作實踐. 
絕知此事要躬行, 實踐是艱難且不可避免的, 卻也是最有趣的. 

我基於 GNU 自由文檔許可證發布此書, 
也就是說, 任何人都可以基於此協議, 複製, 修改, 分享此書. 
\index{GNU Free Documentation License}
\index{Free Documentation License, GNU}

後續的事情就有趣了, 弗吉尼亞州的Jeff Elkner高校教師採用了本書, 
並轉換為了Python教材. 通過他給我的Python版書籍, 
我有了一段不一樣的Python學習之旅. 
同時依靠 Green Tea Press, 於2001年, 我首次發布了Python版. 
\index{Elkner, Jeff}

2003年, 我開始在歐林學院任職, 並且首次教授Python. 
和教授Java相比, 驚喜非凡, 學生樂於學習且樂在其中. 
\index{Olin College}

自此, 我不斷優化此書, 校正錯誤, 提高案例水準, 
並不斷增加一些新的例子和練習. 

因此本書有個通俗易懂的名字{\em Think Python}. 
部分修改如下:
...
\item 探討了程序開發流程以及基礎設計模式. 

\item 增加了關於調試和算法分析的附錄. 

\end{itemize}

第二版 {\em Think Python} 有如下變化：
...
\item 對於第~\ref{turtle}節, 我將本人常用的海龜繪圖包 Swampy 換成了功能更豐富, 
應用更廣泛的 {\tt turtle}包. 

\item 增加了新章節 "利器", 主要介紹一些 Python 不常用但是高效的功能. 

\end{itemize}

希望本書能對大家在學習編程知識, 以及培養計算機專家一樣思維模式
方面有所裨益. 
...
感謝Jeff Elkner把這本Java書籍轉換為Python, 
並讓我愛上了Python.
\index{Elkner, Jeff}

感謝Chris Meyers寫了 {\em How to Think Like a Computer Scientist} 部分章節. 
\index{Meyers, Chris}

感謝創建GNU自由文檔協議以及創作共用的自由軟件基金會. 
基於此協議以及知識共享協議, 我才能和Jeff, Chris一起合作. 
\index{GNU Free Documentation License}
\index{Free Documentation License, GNU}
\index{Creative Commons}


感謝整理{\em How to Think Like a Computer Scientist}的Lulu的編纂. 

感謝耐心編輯 {\em Think Python}的O'Reilly Media的朋友. 

感謝使用本書早期版本的學生們, 感謝所有提出建議和提出錯誤的朋友. 
...
\item Beno\^{i}t Girard 修正了5.6節一個重大錯誤. 

\item Courtney Gleason 和 Katherine Smith 編寫了 {\tt horsebet.py}, 
這是本書早期版本的一個案例腳本, 此程序現在仍然可以在網站找到. 

\item Lee Harr 修正了多個錯誤, 所以應該把他列為主要編輯者. 
...
\item Matthew J. Moelter 作為一個長期貢獻者, 為本書提供了大量修訂和建議. 
...
\item Kevin Parks 為本書的發行優化提供了寶貴的意見. 
...
以及Jonah Cohen 的生成HTML的Prel腳本中的一個錯誤. 
...
\item Peter Winstanley 指出了拉丁版第3章的一個長期存在的錯誤. 
...
\item James Mayer 修正了一堆拼寫和排版問題, 甚至包括貢獻列表中的兩個問題. 
...
\item Gregor Lingl 在奧地利的維也納的一個高校教授Python, 他翻譯了德語版, 並在翻譯過程中, 修正了第5章的多個錯誤. 
...
\item Ben Logan 修正了本書HTML版的大量錯誤. 
...
Max 是 {\em Concrete Abstractions} 一書的作者之一, 讀完此書, 
你可以閱讀這本非凡之作. 
...
的另外一種方案, 並修正了多個錯誤. 同時他知曉如何拼寫Jane Austen. 
...
\item Eric Bronner 指出了關於執行順序的討論中的一處歧義. 
...
\item Andrea Zanella 將此書翻譯為意大利文, 同時修正了多個錯誤. 
...
\item Laurent Rosenfeld 和 Mihaela Rotaru 將本書翻譯為了法文, 並糾正了諸多錯誤. 
...
比如數學, 計算機專家用專業語言去表達觀點(尤其涉及計算). 
比如工程師, 他們設計原件, 拼裝組件, 構建系統, 權衡各種方案. 
比如科學家, 他們觀察複雜系統行為, 小心假設, 積極求證. 
\index{problem solving}

計算機專家最重要的能力是{\bf 解決問題}. 
解決問題意味著要分析問題, 提出創造性解決方案, 並能清晰準確表述. 
事實證明, 學習編程是鍛鍊解決問題能力的極好機會. 
這也是本章叫做程序思維的緣由. 
 
一方面, 我們將學習編程這一技能, 另一方面, 我們將利用編程技能來實現目標. 
隨著不斷學習, 我們終將看到彼岸. 

\section{何為程序?}

{\bf 程序} 簡而言之, 就是一堆執行運算的指令. 
...
\item[輸出:] 將數據顯示於屏幕, 保存至文件, 發送到網絡等. 

\item[數學計算:] 加減乘除等數學運算. 

\item[條件選擇:] 只能在命運分支的安排下, 低頭前進. 

\item[循環執行:] 在宿命輪迴中不斷嘗試, 於既定界限處終達彼岸. 
...
巧婦難為無米之炊, 運行Python程序必然需要先安裝Python和其他軟件, 
熟悉操作系統和命令行的人還好, 對於初學者, 同時學習操作系統和編程, 
必然是件萬分痛苦的事情. 
\index{running Python}
\index{Python!running}

我建議先在瀏覽器上進行Python編程學習, 後續再按步驟進行Python安裝. 
\index{Python in a browser}

在線運行Python代碼的網站很多, 如果無從下手, 我推薦PythonAnywhere. 
入門指導可以查閱\url{http://tinyurl.com/thinkpython2e}
\index{PythonAnywhere}

本書有兩個版本, 分別是 Python 2 和 Python 3, 內容相似, 
可以互相借鑑. 其中差異, 初學時需要關注. 
本書主要面向 Python 3, 同時包括了一些關於Python 2的內容. 
\index{Python 2}

運行Python代碼的程序叫做 Python {\bf 解釋器} . 通常點擊圖標
或者在命令行輸入{\tt python} 來啟動程序. 啟動後, 可以看到如下
輸出：
\index{interpreter}
...
前三行主要是 Python 解釋器信息以及操作系統信息, 內容因人而異. 
注意版本信息, 比如 {\tt 3.4.0}, 第一位的3表示此解釋器版本是Python 3, 
如果是2, 則表示Pyhton 2. 
...
通常, 學習一門新的編程語言, 遇到的第一個程序就是 "Hello, World!". 
Python語言這樣表示：
...
單引號表示語句的開始和結束, 並不會顯示在輸出上. 
\index{quotation mark}
\index{print statement}
\index{statement!print}

括號表示{\tt print}是個函數, 後續在第~\ref{funcchap}節會介紹. 
\index{function} \index{print function}

Python 2中, print 語法明顯不同, 它不是函數, 所以後邊不會有括號. 
\index{Python 2}
...
講完了 "Hello, World", 我們講講算術. Python 通過 {\bf 運算符} 
實現加減乘除等計算. 

運算符 {\tt +}, {\tt -}, 和 {\tt *} 分別表示加法, 減法和乘法, 參看以下樣例：
...
運算符 \verb"^" 在某些編程語言中表示冪運算, 但是Python中, 
...
\item[程序:] 電腦程序的編寫不允許有任何比喻象徵等手法, 必須
遵照語法結構, 不可有一絲逾矩. 
...
\item[可移植(portability):] 一個程序可運行於多種設備的特性. 
\index{portability}

\item[解釋器(interpret):] 讀取並執行代碼的程序. 
\index{interpret}

\item[提示符(prompt):] 解釋器的聲明, 用來提示準備接收輸入信息. 
\index{prompt}

\item[程序(program):] 運算指令集合. 
\index{program}
...
\item[運算符(operator):] 類似加減乘除的符號. 
\index{operator}

\item[值(value):] 程序操作的基本元素, 例如數字或者字符串. 
\index{value}
...
\item[符號(token):] 程序中的基本元素, 類似自然語言中的單詞. 
\index{token}

\item[語法(syntax):] 程序的結構規範. 
\index{syntax}

\item[解析(parse):] 分析語法結構並檢驗程序. 
\index{parse}
...
學習新知識更有效的方式是不斷試錯, 不斷思考. 比如 "Hello, World" 的程序, 
試一試雙引號只寫一半是否可以？
不寫雙引號, 會怎麼樣？
以及 {\tt print} 拼錯了, 又會如何？
\index{error message}
...
程序工程師一般會為變量起個容易理解記憶的名字, 使人一看即知其用途. 
...
當表達式含有多個運算符時, 執行順序主要基於 {\bf 運算次序}. 
對於數學運算符, Python也遵循數學慣例. 
...
不必強求記憶這些運算符的優先級, 碰到比較複雜的表達式, 採用括號會方便很多. 
...
通常, 字符串甚至類似數字的字符串的運算, 一般不能使用數學運算符, 
下面的表達式均是非法運算：
...
所以, 在程序代碼中加入一些自然語言來進行解釋, 說明其作用, 便顯得異常重要. 
...
如果程序存在語法錯誤, Python會直接報錯並退出執行, 
程序便無法繼續運行. 
開始學習編程時, 需要耗費大量精力跟蹤語法錯誤. 
隨著經驗增長, 此類錯誤會越來越少, 而定位也會越來越快. 


\item[運行時錯誤:] 運行時錯誤只有在程序開始運行後才會出現. 
...
\item[操作數(operand):]  運算符操作的單一值對象. 
\index{operand}

\item[表達式(expression):]  變量、運算符以及值的集合, 共同來表示單一結果. 
\index{expression}
...
\item[運算次序(order of operations):]  包含多運算符和操作數的表達式執行時所遵循的先後順序. 
\index{order of operations}

\item[拼接(concatenate):]  操作數首位相接. 
\index{concatenation}

\item[注釋(comment):]  幫助他人理解代碼的有效信息, 對程序執行無影響. 
\index{comment}

\item[語法錯誤(syntax error):] 導致代碼無法解析(因此無法解釋)的錯誤. 
\index{syntax error}

\item[異常(exception):]  程序運行時遇到的錯誤. 
\index{exception}
...
\end{verbatim}
%
此程序包含兩個函數：\verb"print_lyrics" 和 \verb"repeat_lyrics". 
...
練習一下, 將程序的末行提到開始, 令函數先被調用, 再被定義. 
運行程序, 看看會報什麼錯誤. 

將函數調用再次移回末尾, 然後將 \verb"print_lyrics" 函數定義
放到 \verb"repeat_lyrics" 函數的後面, 看看程序運行時, 會發生什麼?
...
\index{frame}
若想跟蹤變量使用情況, 一般繪製{\bf 棧圖}比較有效. 
和狀態圖類似, 棧圖也會標識每個變量的值, 不同之處是, 
棧圖會標識變量所屬函數. 
\index{stack diagram}
\index{diagram!stack}
...
將程序劃分為一個個函數是否值得, 對此的考量, 可以參看以下緣由:
...
調試又像實驗科學, 針對問題, 一旦有所想法, 便可修改程序, 再次嘗試. 
如果假設正確, 便可預期修改後的結果, 向可執行的程序代碼推進. 
...
對某些人而言, 編程便是調試. 這是因為, 編程便是不斷調試程序, 直到實現目標的過程. 
所以, 建議先寫可執行代碼, 然後逐步修改並調試程序, 進而實現預定效果. 

比如, Linux 操作系統具有數百萬行代碼,  但它起源於Linus Torvalds
研究Intel 80386芯片的一段簡單代碼. 據Larry Greenfield所說, 
``Linus的早期項目中, 有個程序是在AAAA和BBBB之間交替輸出, 而這後來演化為了Linux.''
...
\item[函數調用(function call):] 運行函數的語句, 由函數名, 以及括號中的參數列表構成. 
\index{function call}
...
\item[棧圖(stack diagram):]  表示函數,變量,以及值的關係圖. 
\index{stack diagram}

\item[框(frame):]  棧圖中表示函數調用的箱體圖,包括局部變量以及參數. 
\index{function frame}
\index{frame}
...
\end{verbatim}
%
運行程序, 可以看到{\tt bob}對象會向東移動, 然後向北移動, 並留下兩根垂直線段. 
...
函數體可以是任意行語句. 
 ...
 {\tt for} 語句也叫 {\bf 循環}, 因為代碼執行流程會重複執行循環體, 
此例中, 會執行四次循環體.
\index{loop}

這一版代碼和上一版略有不同, 繪製完最後一條線後, 會多一次轉向. 
雖然多的這一步操作增加了耗時, 但是利用循環, 使重複操作更加簡單. 
...
編寫函數, 將{\tt bob} 作為參數傳給{\tt square}, 然後運行程序. 

\item 為 {\tt square}添加參數{\tt length}, 令邊的長度為{\tt length}, 
同時修改函數, 令其調用第二個實參. 再次運行程序, 給{\tt length} 賦不同的值進行測試. 
...
用不同的 {\tt r}值, 測試程序. \index{circle function} \index{function!circle}
...
\end{verbatim}
%
函數內的 {\tt fd} 和 {\tt lt}縮進兩次, 表示運行於函數定義裡面的{\tt for} 循環中. 
下一行 {\tt square(bob)}, 左邊距對齊同時沒有縮進, 
表示上面的 {\tt for} 循環以及函數定義都已結束. 
...
這樣用戶( {\tt circle}的調用者)便可以更方便地使用, 但是接口會顯得不那麼簡潔了. 
\index{interface}

函數{\bf 接口} 是函數用途的簡介: 參數是什麼？函數做什麼？返回什麼？
如果一個接口聚焦於特定功能, 而非無關細節, 那麼這個接口就是``簡潔''的. 

這個案例中, {\tt r}放在接口中合適, 是因為要用它來確定圓的大小. 
而{\tt n}放進接口則顯得不太友好, 是因為它只與{\em 如何}繪製圓的細節有關. 

與其令接口複雜, 不如根據{\tt 周長}來確定一個合適的{\tt n}:
...
\end{verbatim}
%
函數後半段和 {\tt polygon}很像, 但如果我們不調整接口, 便無法復用 {\tt polygon}. 
...
\item 嘗試重構程序. 比如, 在多個地方有相似代碼, 試試能否將其
拆解為恰當的通用函數.

\end{enumerate}

這種流程存在一定的缺點---後續會講述一些替代方案---但是在你無法提前規劃, 
如何將程序拆解為函數時, 這種方法可以讓你再編碼的同時優化程序. 

\section{幫助文檔}
\label{docstring}
\index{docstring}
{\bf 幫助文檔}是指函數開頭部分, 用來對接口(``doc''是``documentation''的簡稱)
進行解釋說明的文檔. 下面是個例子:
...
設計接口, 重要的一步, 便是編寫友好的幫助文檔. 
一個設計優良的接口, 應該是易於理解的;
如果需要耗費大量口舌講解, 那麼這個接口還有待優化. 
...
\item[接口(interface):] 關於如何使用函數的描述, 包括函數名, 實參, 以及返回值信息. 
\index{interface}

\item[重構(refactoring):] 修改代碼, 以優化函數接口, 提升代碼質量的流程.
\index{refactoring}

\item[開發計劃(development plan):] 編碼流程.
\index{development plan}

\item[幫助文檔(docstring):] 函數定義開頭部分的文檔, 用來描述函數接口. 
\index{docstring}
...
\item 繪製棧圖, 描述{\tt circle(bob, radius)}運行時的程序狀態. 
...
\end{verbatim}
%
雖然這些運算符對你來說並不陌生, 但是Python中的運算符和數學裡的不同. 
一個很常見的錯誤就是混淆單等號({\tt =})和雙等號({\tt ==}). 
要知道,  {\tt =}是賦值運算符, 而{\tt ==}是關係運算符. 
而且也沒有{\tt =<} 和 {\tt =>}這種運算符. 
...
有三種{\bf 邏輯運算符}: {\tt and}, {\tt or}, 和 {\tt not}.
其語義和英文中的意思很相似. 例如
...
最後, {\tt not}運算符是個對布爾表達式取反的操作. 
所以, 當{\tt x > y}為假,  {\tt not (x > y)}便為真, 
也就表示{\tt x}小於或者等於{\tt y}時, 為真.

嚴格來說, 邏輯運算符的操作數都應該是布爾表達式, 不過Python在這方面不太嚴格. 
任何非零的數都當作{\tt True}:
...
若想寫出實用性強的程序, 就必然需要程序可以根據條件, 進行選擇處理. 
{\bf 條件語句} 便可以解決此難題. 
最簡單的便是{\tt if} 語句:
...
邏輯運算符可以有效簡化嵌套條件語句, 例如, 可以用一行條件語句來重寫下面的代碼:
...
\end{verbatim}
%
只有兩個條件都滿足, {\tt print} 語句才會運行, 這恰恰和{\tt and}運算符的作用一樣:
...
對於這種簡單樣例, 用{\tt for} 循環可能更加方便. 
但是後續我們會遇到一些例子, 用{\tt for}循環比較難編寫, 而用遞歸卻輕而易舉, 
所以, 現在不啻為一個好的開始. 
\index{for loop}
\index{loop!for}
...
在第~\ref{stackdiagram}節, 我們使用棧圖來描述函數調用時
程序的狀態. 
同樣, 棧圖也有助於我們更好理解遞歸函數. 
...
圖~\ref{fig.stack2}便是以{\tt n = 3}為參數, 調用{\tt countdown}所產生的棧圖. 
...
四個{\tt countdown}的框分別對應不同的{\tt n}值, 最底層{\tt n=0}的棧, 叫做
{\tt 邊界條件},  也就是不再進行遞歸調用的棧, 所以下面也不會再有其他框了. 

做個練習, 以\verb"s = 'Hello'"和{\tt n=2}為參數, 繪製\verb"print_n"調用的棧圖. 
...
\end{verbatim}
%
這次的追蹤信息比以往的要長一些. 這個錯誤發生時, 棧中已經有1000個{\tt 遞歸}框了!
...
到目前為止, 我們編寫的程序,基本都沒有涉及用戶輸入, 它們每次執行都一樣. 

Python提供了內置函數{\tt input},  可以暫停程序運行, 等待用戶鍵入信息. 
當用戶敲擊{\sf  Return} 或 {\sf Enter}鍵時, 程序恢復運行, 
...
\item[向下取整(floor division):] 運算符{\tt //}, 將兩數相除, 結果向下(負無窮方向)取整.
  \index{floor division} 
  \index{division!floor}

\item[求模運算符(modulus operator):]  百分號({\tt \%})表示的運算符, 用於整數, 表示兩數相除後的餘數. 
\index{modulus operator}
\index{operator!modulus}
...
\item[關係運算符(relational operator):] 比較操作數的運算符: 
{\tt ==}, {\tt !=}, {\tt >}, {\tt <}, {\tt >=}, 和 {\tt <=}.

\item[邏輯運算符(logical operator):]  拼接布爾表達式的運算符: 
{\tt and}, {\tt or}, 以及 {\tt not}.

\item[條件語句(conditional statement):]  根據條件, 確定程序運行流程的語句.
\index{conditional statement}
\index{statement!conditional}
...
\[a^n + b^n = c^n \]
%
那麼程序應輸出,  ``Holy smokes, Fermat was wrong!'', 
...
有值返回函數中, 最好保證程序中的每種可能情況, 都有{\tt return}語句. 例如:
...
若想解決越來越複雜的程序, 可以試試新的方法, 即{\bf 增量開發}.
...
{\tt print} 語句只是為了便於調試代碼, 一旦確認程序無誤, 就應將其移除. 
類似這種代碼, 一般叫做{\bf 腳手架代碼},  主要用來輔助構建程序, 而並不應
忝列最終成品之中. 
\index{scaffolding}
...
\item 首先寫個可運行的程序, 然後逐步增加. 任何時候遇到錯誤, 都要弄清緣由, 盡快解決. 

\item 利用變量保存中間狀態值, 以便展示和檢驗. 

\item 一旦程序運行正常, 便可以移除冗餘代碼, 並精簡繁瑣的語句. 
但是要警惕不要使代碼難以閱讀和理解. 
...
\end{verbatim}
%
臨時變量{\tt radius} 和 {\tt result} 在開發和調試時有用, 但是程序一旦正常運行, 
便可以通過組合函數調用, 精簡如下:
...
任何程序都可以只用以上所學模塊, 進行構建(當然, 可能還需要一些控制設備
的命令, 比如管理鼠標, 硬盤等, 但是也僅需要這些).

最早證明了上述偉大結論的是艾蘭圖靈(Alan Turing), 最早的計算機科學家之一
(有人會計較其是數學家, 但是早期的計算機科學家, 基本都是數學家). 
...
遞歸定義和循環定義類似, 某種意義上說, 函數定義體內包含了對定義體的引用. 
通常一個完全循環的定義, 是無用的:
...
圖~\ref{fig.stack3} 是這一系列函數調用的棧圖展現.
...
此程序展示了一種叫做{\bf 哨兵}的角色. 
前兩個條件, 就像哨兵一樣, 避免程序犯錯, 使其正確運行. 
...
\section{調試}
\label{factdebug}
將程序大而化小, 也就天然地為調試創造了一個個的檢查點. 
如果程序運行異常, 需要考慮以下三種可能原因:
\index{debugging} 
...
\item[無效代碼(dead code):]  程序永遠不會運行的語句, 通常在{\tt return}語句之後. 
\index{dead code}
...
\item[腳手架(scaffolding):]  程序開發中起輔助作用的代碼, 但不會留在最終程序中. 
\index{scaffolding}
...
在第~\ref{repetition}節的{\tt for}循環, 也是一種疊代. 
...
圖~\ref{fig.assign2}  展示了棧圖中{\bf 重新賦值}
...
這種程序流轉, 便叫做循環. 因為第三步驟時, 會返回到起點. 
\index{condition}
\index{loop}
\index{body}

循環體通常會修改一個或多個變量的值, 從而令條件最終為假, 終止循環. 
否則, 循環會一直重複, 成為{\bf 無限循環}. 
對計算機科學家們, 有個樂此不疲的玩笑, 
便是觀察洗髮水的使用說明, 
``起泡, 沖洗, 重複'',  這就是個無限循環. 
\index{infinite loop}
...
在{\tt countdown}例子中, 我們如此保證循環終止：
如果{\tt n}小於或等於0,循環不再運行. 
由於{\tt n}每循環一次, 就會變小, 終究會變成0.
...
\end{verbatim}
%
此循環的條件是{\tt n != 1},  那麼只有{\tt n}等於{\tt 1}時, 條件為假, 循環才終止. 

每次循環, 程序都輸出{\tt n}的值, 然後檢查是奇是偶. 
如果是偶數, 則{\tt n}除以2. 如果是奇數, 則{\tt n}值更新為{\tt n*3 + 1}. 
例如, 如果{\tt sequence}的參數為3, {\tt n}值依次變為3, 10, 5, 16, 8, 4, 2, 1.


由於{\tt n} 有時增加, 有時減少, 所以很難保證{\tt n}會達到1, 或者程序終止. 
對於某些特殊的{\tt n}值, 我們可以確信循環會終止. 
比如, 如果起始值是2的冪, 那麼循環每次執行後{\tt n}都會是偶數, 直到最終成為1.
剛剛例子中得到的數列, 從16開始, 便是如此. 
\index{Collatz conjecture}

問題的難點在於, 是否可以證明對於{\em 所有}正數的{\tt n}, 程序都會結束. 
目前, 還沒有人能證明或者證否此命題! 
...
有時, 想要出紅塵, 就需要先入紅塵, 只有進入循環體, 
才知道何時應當終止循環. 
這時, 我們可以用{\tt break}語句跳出循環. 

例如, 想要獲取用戶的輸入, 直到用戶輸入{\tt done}, 才跳出循環, 可以這樣寫:
...
\end{verbatim}
%
循環條件為{\tt True}, 便會一直循環, 所以只有觸及 break 語句, 才會跳出循環. 

每次循環, 都會打印尖括號來提示用戶輸入, 
如果用戶輸入了{\tt done}, {\tt break} 語句會終止循環. 
否則, 程序會打印用戶的輸入, 並進入下一輪循環. 
舉個例子:
...
在編程中, 循環通常用來進行數值計算, 通過用近似值逼近真實值來實現. 
...
\end{verbatim}
%
當{\tt y == x}時, 我們便可以停止循環了. 
下面的循環, 以估計值{\tt x}開始並不斷逼近真實值, 在結果不再變化時終止:
...
或者, 將程序對半切分. 找到程序的中間位置, 或者靠近中間的位置, 
...
\item[疊代(iteration):] 採用遞歸或者循環, 重複執行一段語句. 
\index{iteration}

\item[無限循環(infinite loop):] 終止條件一直不能抵達的循環. 
\index{infinite loop}
...
複製第~\ref{squareroot}節的循環, 封裝為\verb"mysqrt" 函數, 
...
同時返回結果. 用{\tt while}循環計算各個項並求和, 直到最後一項小於{\tt 1e-15}
($10^{-15}$的python表示法)為止. 
...
而不是{\tt a}. 但是, 對於計算機科學家來說, 索引是從字符串開始位置的偏移量, 
所以第一個字符的偏移量是0.
...
\end{verbatim}
%
上面循環會遍歷字符串, 並每行打印一個字符. 
循環條件是{\tt index < len(fruit)}, 
所以當{\tt index} 等於字符串長度時, 條件為假, 循環體終止. 
最後獲取的字符, 索引為{\tt len(fruit)-1}, 也就是最後一個字符. 

做個練習, 寫個函數, 以字符串為入參, 倒序輸出每個字符, 每行一個. 

遍歷字符串的另一種方法是用{\tt for}循環:
...
\end{verbatim}
%
每次循環後, 字符串中下一個字符會賦值給變量{\tt letter}. 
直到沒有字符可用, 循環便終止. 
\index{concatenation}
\index{abecedarian}
\index{McCloskey, Robert}

下面的例子, 展示了如何用拼接(字符串相加)以及{\tt for}循環, 來構建
一個簡單序列(按照字母順序). 
...
這是我們第一次見到, 在循環內使用{\tt return}語句. 
如果{\tt word[index] == letter}, 函數會跳出循環, 並立刻返回結果. 

如果字符串中沒有想要的字符, 程序會一直執行到循環結束, 並
返回{\tt -1}.
...
\end{verbatim}
%
此程序描述了另一種算法, 稱之為{\bf 計數器}.
初始化變量{\tt count}為0, 其後每找到一次{\tt a}, 就加1.
循環結束後,  {\tt count}便包含了結果-- {\tt a}的全部數量.
...
讀一下這個循環, ``for (each) letter in (the first) word, if (the) letter 
...
\end{verbatim}
%
這次我們的到了正確結果, 但是循環只運行了三次, 有點奇怪. 
為了弄清怎麼回事, 可以繪製棧圖來輔助理解. 
...
從此狀態框開始, 在紙上執行程序, 每次疊代時修改{\tt i} 和 {\tt j}值. 
然後找到並修復函數中的第二個錯誤. 
\label{isreverse}
...
\chapter{案例學習: word play}
\label{wordplay}

本章學習第二個案例, 主要研究如何通過搜索特定詞彙, 進行猜謎. 
比如, 查找最長回字文, 以及尋找按照字母表順序排列的單詞. 
同時, 我將介紹一種新的程序開發模式: 抽離紛紛擾擾, 回歸已有方案. 


\section{讀取單詞列表}
\label{wordlist}
本章的練習, 需要準備一個英文單詞列表. 
網上有很多可用的單詞列表, 但是對我們來說, 最理想的莫過於
Grady Ward 收集整理, 作為Moby詞典項目, 貢獻給公共領域的單詞列表
(詳見\url{http://wikipedia.org/wiki/Moby_Project}). 這是
包含 113,809 個填詞遊戲的單詞列表; 也就是說, 這些單詞, 已經被
填詞遊戲和其他單詞遊戲證明了有效. 
...
\end{verbatim}
%
你也可以將文件對象置於{\tt for}循環中. 
這樣程序便會讀取 {\tt words.txt}, 然後逐行輸出每個單詞:
\index{open function}
\index{function!open}
...
\begin{exercise}
編寫程序, 讀取{\tt words.txt}, 僅打印多於20個字符的單詞
(不包括空格). 
\index{whitespace}
...
編寫程序, 讀取{\tt words.txt}, 只打印不包含``e''的單詞. 
統計列表中, 不包含``e''的單詞所占比例. 
\index{lipogram}
...
\end{verbatim}
%
{\tt for}循環會遍歷 {\tt word}中所有字母. 
如果遇到 ``e'', 則即刻返回{\tt False};
否則繼續下一個字母. 
如果循環正常結束, 也就是說沒有遇到``e'',  則返回{\tt True}.
\index{traversal}

\index{in operator}
\index{operator!in}
你也可以使用{\tt in}運算符來精簡程序, 
...
\end{verbatim}
%
這裡不再是禁用字母列表了, 而是可用字母列表. 
如果 {\tt word}中出現了不在可用列表中的字母, 返回{\tt False}.
...
不再是遍歷 {\tt word}中的字母, 
而是遍歷必需字母列表. 
如果字母列表中有字母未出現在單詞中, 返回{\tt False}.
\index{traversal}

如若你已像計算機科學家一樣去思考, 你便會注意到 \verb"uses_all" 
...
\end{verbatim}
%
此案例便是一個{\bf 抽離紛擾, 回歸已知}的程序開發模式的實踐, 
...
前一章節中, 我用{\tt for}循環編寫了大量函數, 這是因為我
只關注字符串中的字符; 所以無需關注索引. 
...
\end{verbatim}
%
此循環始於 {\tt i=0}, 終於{\tt i=len(word)-1}. 
每次循環都會比較第$i$個字符(可以看作當前字符)
和第$i+1$個字符(可以看作後一個字符). 

如果後一個字符小於(在字母表順序中先於)當前字符, 
我們會發現這和要求不符, 便返回{\tt False}.

如果直到循環末尾, 仍然無異常, 則單詞合格. 
為確信循環正常結束, 可以試試\verb"'flossy'". 
單詞長度為6, 所以{\tt i} 為 4時, 便是最後一次循環, 
因為這是倒數第二個字符的索引. 最後一次循環中, 
...
程序測試很難. 本章的函數相對容易測試, 因為你可以用手算來檢驗結果. 
...
除了自己構建測試用例, 你也可以用類似{\tt words.txt}的單詞列表檢驗程序. 
通過掃描輸出, 你可能會發現錯誤, 但要注意: 
你可能發現了某種錯誤(包括了不應包含的單詞), 而忽略了其他錯誤
(沒有包括應該包含的單詞). 

通常, 程序測試可以幫助你發現錯誤, 但要製作出好的測試用例, 
往往很難, 即使通過了這些測試, 你也不能百分百地確認你的程序正確. 
一位傳奇的計算機科學家說過:
\index{testing!and absence of bugs}

\begin{quote}

程序測試只能表明錯誤的存在, 卻永遠無法保證其不存在!
...
編寫個Python程序, 檢驗所有的6位數字, 然後輸出滿足上述要求的任意數字. 
...
同字符串一樣, {\bf 列表} 也是值的序列. 
字符串中, 值是字符;在列表中, 值可以是任何類型. 
列表中的值, 叫做{\bf 元素}, 有時也叫{\bf 列表項}.
\index{list}
\index{type!list}
\index{element}
\index{sequence}
...
創建列表的方式很多; 最簡單的莫過於將元素用方括號包起來(\verb"[" and \verb"]"):
...
從列表中獲取元素的語法和從字符串中獲取字符的語法一樣--方括號操作符. 
括號中的表達式確定了索引值. 
要注意, 索引從0開始:
...
列表用單詞``list''在外, 元素在內的箱體圖表示. 
{\tt cheeses} 指向了一個包括三個元素的列表, 其索引分別為0,1和2. 
...
遍歷列表中元素的最常用方法便是{\tt for}循環. 語法和字符串遍歷相同:
...
\end{verbatim}
%
這種方法對於僅讀取列表中元素很方便, 但是如果你想寫入或者更新元素, 
那便需要索引了. 
...
此循環會遍歷列表並更新每個元素. 
{\tt len}方法會返回列表中元素數量. {\tt range} 則返回從0到 $n-1$的索引值列表, 
其中 $n$ 是列表的長度. 
每次循環, {\tt i}都會獲得下一個元素的索引. 循環體中的賦值語句
則會通過{\tt i}獲取元素的舊值, 並賦予新值. 
\index{item update}
\index{update!item}

用{\tt for}循環遍歷空列表, 永遠不會執行循環體:
...
\end{verbatim}
%
因為列表可變, 所以最好在執行修改列表的操作前, 複製一份, 以防萬一. 
\index{mutability}

如果切片運算符在賦值號左側, 便可以同時更新多個元素:
\index{slice!update}
\index{update!slice}
...
\end{verbatim}
%
{\tt extend}會以列表為參數, 並將其中所有元素添加到執行列表中:
\index{extend method}
\index{method!extend}
...
\end{verbatim}
%
多數列表方法都沒有返回值; 這些方法修改列表然後返回{\tt None}. 
...
%
{\tt total}初始為0.
每循環一次{\tt x} 會從列表中獲取一個元素. {\tt +=} 操作符, 是一種更新運算的便捷寫法. 
也是一種 {\bf 增量賦值語句}. 
...
\end{verbatim}
%
隨著循環運行, {\tt total} 會累積元素求和;
這樣的變量有時也稱為{\bf 累加器}. 
\index{accumulator!sum}
對列表元素求和, 使用比較普遍, 所以Python提供了內置函數{\tt sum}:
...
有時, 你會遍歷列表, 創建新列表. 例如, 下述函數會接收字符串列表, 
並返回一個將所有字符串都轉為首字母大寫的字符串列表:
...
\end{verbatim}
%
{\tt res} 開始是一個空列表; 每次循環, 都會向其中添加下一個元素. 
...
另一種常用操作是, 從列表中篩選部分元素, 返回子列表. 例如, 
下面函數會接收一個字符串列表, 返回僅包含大寫字母的字符串列表:
...
大部分的列表操作都可以由map, filter 以及reduce 組合構成. 
...
字符串是一系列的字符, 列表是一系列的值, 但是字符列表和字符串並不可等同視之. 
採用 {\tt list}方法, 可以將字符串轉換為字符列表:
\index{list!function}
\index{function!list}
...
若想判斷兩個變量是否指向了同一個對象, 可以用{\tt is}運算符.
...
一個對象有一個值. 若創建{\tt [1, 2, 3]}, 你會得到一個列表對象, 
而這個對象的值是個整數序列. 
如果另外一個列表也有相同的元素, 我們便說它們擁有相同的值, 
但它們不是同一個對象. 
\index{object}
\index{value}
...
當給函數傳遞列表時, 函數收到的是對該列表的引用. 
如果函數修改了列表, 那麼調用一方也會看到相應變化. 
例如, \verb"delete_head" 函數刪除了列表中第一個元素:
...
因兩個框共用一個列表, 所以我把列表畫在了它們中間. 

區分修改列表操作和新建列表操作, 是非常重要的. 
例如,  {\tt append}方法是修改列表, 而{\tt +}運算符則會新建列表.
\index{append method}
\index{method!append}
\index{list!concatenation}
\index{concatenation!list}
...
在為修改列表而編寫函數時, 此差異尤要重視. 
例如, 下面函數{\em 沒有}刪除列表的第一個元素:
%
\begin{verbatim}
...
列表. 函數結束時, {\tt t}指向了新的列表, 
但是{\tt t4} 仍然指向了原來的,未被修改的列表.

一種替代方案是, 編寫一個創建並返回新列表的函數. 比如, 
{\tt tail}函數會返回列表中除首元素的所有元素列表:
...
若使用列表(和其他可變對象)時不夠小心謹慎, 很容易導致長達數小時的調試跟蹤. 
這裡有一些常見陷阱以及如何避免:

\begin{enumerate}

\item 多數的列表方法都是傳入參數, 返回 {\tt None}. 
...
在使用列表方法前, 你應該認真閱讀文檔, 並在交互模式下測試一下. 

\item 確定原則, 堅定執行.

列表使用中面臨的部分問題在於, 有多條道路通羅馬. 
比如, 從列表中移除元素, 你可以用 {\tt pop}, {\tt remove}, {\tt del},
甚至用切片重新賦值. 

若想給列表添加元素, 可以用{\tt append}方法, 或者{\tt +}運算符. 
假設{\tt t}是列表, {\tt x}是列表元素, 下面的操作都正確: 
...
你若想使用{\tt sort} 方法修改參數, 同時又想保留原列表, 那你可以複製一份. 
...
\item[列表(list):] 由值構成的序列. 
\index{list}

\item[元素(element):] 列表(或序列)中的值, 也可以稱為列表項.
\index{element}

\item[嵌套列表(nested list):] 列表中的元素是另外的列表. 
\index{nested list}

\item[累加器(accumulator):] 在循環中用於累加或累積結果的變量.
\index{accumulator}
...
\item[filter:] 一種遍歷列表, 並篩選滿足特定條件的元素的處理模式.
\index{filter pattern}
\index{pattern!filter}
...
編寫 \verb"nested_sum" 函數, 接收包含整數列表的一個列表, 
然後將所有內嵌列表的元素相加求和. 
比如:
...
編寫函數{\tt cumsum} , 使其接收一個數字列表, 返回累加之和;
也就是, 第$i$個元素是原列表中的第$i+1$個元素之前所有元素之和. 
比如:
...
編寫\verb"middle"函數, 接收一個列表, 返回包含掐頭去尾後所有元素的新列表. 
比如:
...
\begin{exercise}
編寫函數 \verb"chop" , 使其接收一個列表, 然後移除首尾元素, 返回 {\tt None}.
例如:
...
\begin{exercise}
編寫 \verb"is_sorted"函數, 接收一個列表, 如果列表是升序排列, 則返回 {\tt True}, 
否則返回 {\tt False}. 例如:
...
編寫\verb"has_duplicates" 函數, 使其接收一個列表, 如果列表中
存在重複出現的元素, 返回{\tt True}. 注意不要修改原始列表.
...
編寫函數讀取{\tt words.txt} 內容, 並將其中每個單詞放入列表中. 
...
\index{binary search}
\index{search, binary}
若要檢驗一個單詞是否在列表中, 可以用{\tt in}運算符, 
但是速度會慢, 因為它是從頭開始, 順序檢索. 

由於這些單詞通常是按照字母表順序排列, 我們可以通過對摺查找
(也叫二分查找), 提高速度. 此方法和你通過字典(此處指書籍, 不是數據結構)
查找單詞的方式很像. 
你一般會先翻到字典中間, 然後看單詞在之前, 還是之後. 如果在前面, 則繼續此
方法查找, 如果在後面, 同樣操作. 

無論怎樣, 你都可以將搜索區間減半. 
如果單詞列表有113,809 個單詞, 大約17次, 你就可以定位到單詞, 
或者確定其不在其中.

編寫函數\verb"in_bisect",  使其接收一個有序列表, 以及一個目標值, 
當單詞在列表中時, 返回{\tt True},  否則返回{\tt False}. 
\index{bisect module}
\index{module!bisect}
...
\end{verbatim}
%
運算符{\tt in}在列表和字典中使用的算法不同. 
對於列表來說, 它會如第~\ref{find}節一樣, 按照順序搜索列表中的元素. 
隨著列表變長, 確定目標的耗時也會更長. 

Python中的字典則使用了叫做{\bf 哈希表}的數據結構, 使其具有了一個神奇特性: 
無論字典中有多少項, {\tt in} 運算符都耗時相同. 
...
\item 你也可以創建一個包含26個元素的列表. 
把每個字母轉換成數字(使用內置函數{\tt ord}), 用這些數字作為列表的索引, 
然後累加相應計數器. 
...
函數第一行新建了一個空字典. 
然後用 {\tt for} 循環遍歷字符串. 每次循環中, 如果字符{\tt c}不在字典中, 
...
基於不同需要, 可能你只需要選一個, 也可能需要將所有的放入列表. 
...
如果一直運行到循環結束, 這表示{\tt v}沒有出現在字典的值中, 
所以會引起異常. 
...
%
反向查找比正向查找要慢得多;
如果使用頻繁, 或者字典過大, 都會對程序性能造成影響. 


\section{字典和列表}
\label{invert}

列表可以作為值出現在字典中. 比如, 給定一個字典, 映射了字母和其頻次, 
你想倒過來看看; 也就是創建個字典, 是從頻次映射到字母的. 
然而, 可能有些字母的頻次相同, 那麼這個倒過來的字典中, 每個值都應該是個字母列表. 
...
\end{verbatim}
%
每次循環, {\tt key}都從{\tt d}中獲取一個鍵, {\tt val}獲取對應的值. 
如果{\tt val}不在{\tt inverse}這個字典中, 也就是說以前沒有遇到過, 
那便創建一個新項, 並用{\bf 單元素集}(包含一個元素的列表)來初始化它. 
如果在字典中, 則將此鍵添加到列表中. \index{singleton}
...
但如果是列表, 我會繪製在箱體外部, 只是為了簡單易懂. 
\index{state diagram}
\index{diagram!state}

列表可以是字典的值, 就像圖中所示, 但是不能作為鍵使用. 
下面是擅自使用遇到的錯誤:
\index{TypeError}
\index{exception!TypeError}
...
\end{verbatim}
%
以前我講過, 字典的實現基於哈希表, 意味著其鍵必須是{\bf 可散列的}.
\index{hash function}
\index{hashable}
...
這也就是鍵必須要可哈希的原因, 以及為何像列表一樣的可變類型不能用作鍵的
...
將之前計算過的值存儲起來, 方便後續使用, 這種操作叫做{\bf 緩存}.
下面是 {\tt fibonacci}的``緩存''版本:
...
比如, 有些程序會使用 {\tt verbose}作為標識, 來控制輸出信息的
詳細程度:
...
\end{verbatim}
%
運行此程序, 你會注意到\verb"been_called"的值沒有改變. 
...
\end{verbatim}
%
全局變量是很有用, 但如果你到處使用, 並且還頻繁修改, 那麼程序往往會
難以調試. 
...
可以重新編輯文件, 或者(更優的做法是)修改程序, 只讀取前{\tt n}行. 

如果報錯, 可以將{\tt n}減小為剛好會報錯的最小值, 
然後修正異常, 並逐步增大{\tt n}值, 如此循環往復. 

\item[檢驗概覽及類型:] 不要打印並檢驗全部數據集, 只需輸出數據的概覽:
比如, 字典中項的個數, 或者列表中數字總數量. 

值類型錯誤, 是產生運行時錯誤的常見原因. 
所以調試此類錯誤, 通常打印值的類型即可. 

\item[編寫自檢代碼:]  有時候, 你可以編寫代碼, 自動檢驗異常. 
例如, 你要計算數字列表的平均值, 你可以檢驗一下, 結果是否
比列表中最大值還要大, 或者比最小值還要小. 這叫做``合理性檢驗'', 
是因為要檢查結果是否``不合理''.  
\index{sanity check}
...
\item[哈希表(hashtable):] 實現Python字典的算法. 
\index{hashtable}

\item[哈希函數(hash function):] 哈希表使用的函數, 用來計算鍵的位置. 
\index{hash function}
...
\item[單元素集(singleton):] 只包含一個元素的列表(或者其他序列).
\index{singleton}

\item[調用圖(call graph):] 一種用來展示程序執行流程的圖, 包括每步創建的框, 
以及從調用者指向被調用者的箭頭. 
\index{call graph}
...
然後便可以用{\tt in}運算符快速檢驗某個字符串是否在字典中. 

如果你做過習題~\ref{wordlist1}, 可以比較一下這種實現, 和列表的{\tt in}運算, 
以及二分查找相比, 速度如何. 
...
\begin{exercise}
用緩存的方式重寫習題~\ref{ackermann} 中的Ackermann函數, 
看看是否緩存方式可以使其處理較大參數. 
提示: 不能.
...
令其接收一個列表作為參數, 如果其中存在任意重複對象, 則返回{\tt True}.
...
本章介紹另一種內置類型, 元組, 以及列表、字典和元組如何一起使用. 
同時展示可變長度參數列表的一個特性, 以及聚合和擴展運算符. 
...
\end{verbatim}
%
也有更普遍的用法, 右側可以用各種類型的序列(字符串, 列表或者元組). 
...
%
聚合的對立面是{\bf 擴展}. 
如果你有一個值的序列, 想將其作為多個參數, 傳入函數, 那便可以使用{\tt *}運算符. 
...
疊代器和列表在某些方面比較相似, 但與列表不同的是, 你不能從
疊代器中通過索引獲取元素. 
\index{iterator}

如果你想使用列表的運算符和方法, 可以用zip對象構建列表:
...
\end{verbatim}
%
列表中都是元組; 在這個例子中, 每個元組都包括
字符串中的一個字母, 以及列表中的相應元素. 
...
\end{verbatim}
%
每次循環, Python都會獲取列表中的下一個元組, 並將其元素分別
...
\end{verbatim}
%
字典的{\tt update}方法也可以接收一個元組列表, 然後將其
作為鍵值對添加到已有字典中.
\index{update method}
\index{method!update}
...
\end{verbatim}
%
此循環會遍歷 {\tt directory}中的鍵, 也就是元組. 
同時會將元組中的元素賦值給{\tt last} 和 {\tt first}, 
然後輸出姓名以及對應的電話號碼. 

在狀態圖中展示元組有兩種方法. 
較詳細的版本會展示起索引和元素, 就像列表一樣. 
...
我一直講的是元組列表, 但是本章幾乎所有示例同樣適用於
列表構成的列表, 元組構成的元組, 以及列表構成的元組. 
為了避免枚舉各種組合, 直接叫做序列構成的序列, 便簡單多了. 

很多情形下, 不同類型的序列(字符串, 列表以及元組)可以替換使用, 
那你如何從中擇優呢?
\index{string}
\index{list}
\index{tuple}
...
\item 在某些情形, 比如{\tt return}語句, 語法上創建元組比創建列表更簡便. 
...
因為元組不可變, 所以它們不提供 {\tt sort} 和 {\tt reverse}這種修改現有列表的方法, 
但是Python提供內置函數 {\tt sorted}, 其接收任意序列, 然後返回一個排序後的新列表, 
...
列表, 字典以及元組都是 {\bf 數據結構}的例子;
本章我們開始了解複合數據結構, 像元組列表, 或者用元組作鍵, 
列表作值的字典. 
複合數據結構很有用, 但很容易產生我說的{\bf 格式異常};
就是當數據結構中出現了不恰當的類型, 大小或結構, 導致的錯誤. 
比如, 你想要一個含有一個整數的列表, 而我給了一個單純的整數
(不被列表包含), 那便會出錯. 
\index{structshape module}
\index{module!structshape}

為了調試這種異常, 我寫了一個叫作 {\tt structshape} 的模塊, 
其提供了一個{\tt structshape}函數, 可以接收任意數據結構作為參數, 
返回一個其格式的概述. 
...
\end{verbatim}
%
如果你對你的數據結構感到棘手, 可以試試{\tt structshape}.
...
\item[疊代器(iterator):] 可遍歷序列的對象, 但不適用列表運算符和方法. 
\index{iterator}

\item[數據結構(data structure):] 相關值的集合, 通常由列表, 字典, 元組等組成. 
\index{data structure}
...
\item 編寫程序, 從文件中讀取單詞列表(參見第~\ref{wordlist}節), 
並且輸出所有的變位詞集合. 
...
\end{verbatim}
%
提示: 你或許會想要構建個字典, 將字母集合映射到單詞列表, 
而列表中的單詞都可以用這些字母構成. 問題來了, 你如何表示這些字母集合, 
從而使其能夠作為鍵來使用?

\item 修改上面的程序, 使其先輸出最長的變位詞, 然後輸出次長, 依次類推. 
\index{Scrabble}
\index{bingo}
...
編寫程序, 尋找所有這樣可以縮減的單詞, 然後確定最長的單詞. 

這道題相比其他題有些難, 所以這裡給些提示:

\begin{enumerate}

\item 你可能需要寫個函數, 接收一個單詞, 然後計算得到所有的單詞列表. 
...
\item 我提供的單詞列表 {\tt words.txt}, 不包含單字母單詞. 所以你可能需要添加``I'', ``a''和空字符串. 

\item 若想提升程序性能, 最好緩存住已知的可縮減單詞. 
...
\chapter{案例學習: 數據結構的選擇}

截止當前, 你已學習了Python最核心的數據結構, 
同時也已熟悉了與之相關的一些算法. 
若想後續深入了解算法知識, 可以學習章節~\ref{algorithms}. 
但現在不用太急切; 後續時間方便了, 可以耐心學習. 

本章通過多道習題, 一個案例來讓你思考如何選擇和應用數據結構. 
...
寫個程序, 讀取文件, 把每行拆分成單詞, 去掉空格和標點, 並將其轉為小寫. 
\index{string module}
\index{module!string}
...
修改上述代碼, 使其讀取單詞列表(參見第~\ref{wordlist}節), 
...
\end{verbatim}
%
這個程序會讀取 {\tt emma.txt}, 其中包括了Jane Austen的{\em Emma}小說. 
\index{Austen, Jane}
...
\verb"process_line"用字符串方法{\tt replace}將連字符替換為空格, 然後用
 {\tt split}方法, 將整行拆分為字符串列表. 
然後遍歷列表, 並使用{\tt strip}方法, 移除標點符號, 使用{\tt lower}方法, 
...
若想找到最常用的單詞, 我們可以創建一個元組列表, 
每個元組都包含一個單詞和其頻數, 然後據此將列表排序.

下面函數接收一個詞頻統計, 然後返回一個詞-頻元組列表:
...
但是有些依舊很常用, 確實也應該包含在列表中!

\begin{exercise}
\index{set}
\index{type!set}

Python提供了一種名為 {\tt set} 的數據結構, 其可以滿足多數的集合操作. 
你可以仔細閱讀章節~\ref{sets}, 或者閱讀
\url{http://docs.python.org/3/library/stdtypes.html#types-set}上面的文檔. 

編寫程序, 使用集合減法, 尋找包含在本書但是不在單詞列表中的單詞. 
參考:
...
都需要重新構建和原書一樣大的列表. 
顯然, 更優的方案是只建立一次列表, 然後多次選擇, 但列表依然很大.

一個更高效的方案是:

\begin{enumerate}

\item 使用 {\tt keys} 生成本書單詞的列表.

\item 用詞頻的各步累積之和(參看習題~\ref{cumulative}), 構建列表. 
列表中最後一項, 便是書中單詞總數, $n$. 
  
\item 選擇一個從1到$n$的隨機值. 使用二分搜索(參看習題~\ref{bisection}), 
尋找隨機值在累積值列表中所嵌入位置的索引. 

\item 使用該索引值從單詞列表中確定對應的單詞. 
...
\item 寫個程序, 讀取文本內容, 進行Markov分析. 
結果應該是個從前綴映射到可能後綴集合的字典. 
這個集合可以是列表, 元組或者字典; 取決於你的選擇. 
你可以用兩個詞的前綴測試程序, 但最好想辦法讓程序可以兼容其他長度的前綴. 

\item 在上面的程序中增加一個函數, 基於Markov分析生成隨機文本. 
...
如果是前綴, 最好選擇字符串, 字符串列表或者字符串元組. 

對於後綴, 可以選擇列表; 或者詞頻統計(字典).
\index{implementation}

至於如何抉擇? 首先要考慮無論選擇哪種數據結構, 都不可避免的相應操作. 
對於前綴來說, 我們需要從開頭刪除單詞, 在末尾添加單詞. 比如, 如果現在
前綴是``Half a'', 後面的詞是``bee'',  你需要構建下一個前綴詞, ``a bee''. 
\index{tuple!as key in dictionary}

首選可能是列表, 因為很容易添加和刪除元素, 但我們同時需要
使用前綴作為字典的鍵, 所以列表不合適. 
而元組呢, 無法執行增刪操作, 
但是可以用加法運算符構建新元組:
...
添加新後綴, 對於列表實現或者詞頻統計都同樣簡單. 
而從列表中選擇隨機元素很容易, 但是從一個詞頻字典中操作, 便不太高效
(參考習題~\ref{randhist}). 

目前來說, 我們考慮更多的是實現難易, 
但在選擇數據結構時, 還需要考慮一些其他因素. 
比如運行時間. 總有一些時候, 理論上可以預期某種數據結構就是比
另外一種要快. 比如, 我提到過的{\tt in}運算符, 字典比列表快, 
最起碼元素數量很多的時候, 一目了然. 

但往往你無法提前知曉哪種實現要更快. 一種方法是, 對每種數據結構都
進行實現, 然後運行比較, 看哪種更快. 這個方法叫做{\bf 基準分析}.
更實際一些的方法是選擇一種最容易實現的數據結構, 
然後看是否滿足預期. 如果滿足, 便沒必要優化了. 如果不滿足, 
可以使用一些工具, 像{\tt profile}模塊, 以定位程序最耗時的部分. 
\index{benchmarking}
...
某些情況下, 節省空間也可以提高程序運行速度, 而且在極端情況下, 
內存耗盡, 程序便無法運行. 但對於多數應用來說, 運行時間的考量要優先於存儲空間. 

最後再發散一下: 在本章節, 我已暗示, 我們應該使用一種數據結構進行分析和生成. 
但這二者又是兩個階段, 那麼便可以在分析時用一種數據結構, 然後在生成時, 
轉換為另一種數據結構. 如果生成節省的時間超過了轉換耗費的時間, 
那這種方案便有利可圖. 
...
取消最近的修改, 直到程序正常並且你能理解. 然後開始再次構建. 

\end{description}

新手程序員有時候會在以上某種步驟中徘徊, 而忘記了其他步驟. 
上面的每一步都有其不靈的時候. 
...
這種過程是隨機修改, 直至程序正常. 可想而知, 隨機遊走編程必然會耗費大量時間. 
...
但如果錯誤過多, 或者嘗試修復的程序過於龐大複雜, 
那即使再好的調試方案, 都會失效. 
有時候, 最好的選擇便是回退代碼, 簡化程序, 直到其正常且你能理解. 

新手程序員往往不願意回退代碼, 因為他們無法忍受刪除一行代碼(即使是錯誤代碼). 
可以在刪減代碼前, 將程序複製一份, 這樣會讓你感覺好一些. 
在你想還原的時候, 便可以一次性複製回來. 
...
\item[確定性(deterministic):] 給定相同輸入, 程序執行相同操作的特性. 
\index{deterministic}
...
\item[基準分析(benchmarking):] 在多個數據結構間抉擇的方法, 通過實現多個方案, 
並用可能的輸入作為樣本進行測試. 
\index{benchmarking}
...
一個單詞的``排名''便是其在單詞列表中根據頻次排序後, 所在的位置:
...
本章介紹程序將數據保存在永久存儲中的``持久化''概念, 同時展示如何
使用不同類型的永久存儲, 比如文件和數據庫. 
...
以往我們遇到的多數程序都是一閃而過的, 因為它們運行很快, 也產生了一些輸出, 
但當運行結束, 其數據便會消失不見. 
如果再次運行, 又是從頭開始而已. 

有些程序是{\bf 持久化的}: 其運行時間很長(甚至一直運行);
這些程序至少會將部分數據進行永久儲存(比如, 硬盤);
如果程序中斷後重新運行, 便可以從之前停止的地方, 繼續執行. 

操作系統便是一個持久化程序的例子, 計算機一旦啟動, 便要執行諸多操作;
再比如網絡服務器, 需要一直運行, 等待接收來自網絡的請求. 

程序管理數據的一個最簡單方法是讀寫文本文件. 
之前, 我們已經接觸過讀取文本文件了, 
本章將學習如何使用程序, 將文本寫入文件. 

此外, 我們也可以將程序狀態存儲到數據庫中. 
本章將介紹一個簡單的數據庫, 以及模塊{\tt pickle}, 
它可以很容易存儲程序數據. 
\index{pickle module}
\index{module!pickle}
...
{\tt os.listdir} 會返回指定目錄下的文件列表(以及其他目錄):
...
通常, 捕獲異常是為了讓你有機會修正問題, 或重試程序, 或者至少優雅地結束程序. 
...
{\bf 數據庫}是用來存儲數據的文件. 
很多數據庫都是像字典一樣構建, 從鍵映射到值, 進行數據存儲. 
數據庫和字典之間最大的區別, 在於數據庫存儲在磁盤(或者其他永久存儲中), 
所以程序結束後, 數據依然存在. 
\index{dbm module} \index{module!dbm}

 {\tt dbm} 模塊提供了一個創建和更新數據庫文件的接口. 
下面的例子中, 我將創建一個數據庫來存儲圖片文件的描述. 
\index{open function}
\index{function!open}
...
模式 \verb"'c'" 表示如果數據庫不存在, 則新建一個. 
其結果是一個數據庫對象, 使用方式(很多操作)和字典一樣.
\index{database object}
\index{object!database}

當創建新的項, {\tt dbm}便會更新數據庫文件.
...
並不適用於數據庫對象. 但是可以用{\tt for}循環對其進行疊代, 遍歷對象:
...
你可以用{\tt pickle}來將任何非字符串數據存儲到數據庫中. 
...
任何你能通過shell啟動的程序, 都可以通過Python, 使用{\bf 管道對象}來啟動, 這個對象表示了一個運行中的程序. 
...
\end{verbatim}
%
如果運行程序, 代碼會讀取自身, 並輸出文件中內容的行數, 也就是7.
你也可以像下面這樣導入這個模塊:
...
\end{verbatim}
%
\verb"__name__" 是一個內置變量, 程序一旦開始運行, 便被賦值. 
如果程序作為一個腳本運行, \verb"__name__"的值為\verb"'__main__'";
就像上面例子, 測試代碼便會執行. 
如果程序作為模塊導入, 那\verb"__name__"的值便不是\verb"'__main__'", 
那測試代碼不會執行. 
...
對於多數系統來說, 有諸多程序可以實現格式轉換. 
你可以在\url{http://en.wikipedia.org/wiki/Newline}上查找軟件
(以及閱讀此問題的更多細節). 
或者, 你也可以自己寫一個轉換工具. 
...
\item[持久化(persistent):] 程序可以隨時執行, 同時至少將數據保存到永久存儲中. 
\index{persistence}

\item[格式運算符(format operator):] 一個操作符, {\tt \%}, 連接格式字符串和元組, 
生成一個將元組元素格式化為指定格式的字符串. 
\index{format operator}
\index{operator!format}

\item[格式字符串(format string):] 用於格式運算符的字符串, 內含格式序列.  
\index{format string}
...
\item[捕獲異常(catch):] 使用 {\tt try} 和 {\tt except} 語句來防止異常中斷程序執行. 
\index{catch}

\item[數據庫(database):] 一個文件, 其內容以類似字典的鍵值對應的方式來管理. 
\index{database}
...
\item[shell:] 一個程序, 允許用戶輸入命令, 然後通過啟動其他程序來執行命令. 
\index{shell}

\item[管道對象(pipe object):] 表示一個正在運行的程序的對象, 
允許Python程序運行命令並讀取結果.
\index{pipe object}
\index{object!pipe}
...
\verb"read_anagrams"則可以查找單詞並返回它的字母異位詞列表. 
...
\item 編寫一個程序, 搜索目錄, 並遞歸其所有子目錄, 
返回一個包括所有給定後綴 (比如 {\tt .mp3})的完整路徑的列表. 
...
別名的使用會讓程序變得複雜, 因為一處改動可能會影響到其他地方. 
...
{\tt is}運算符表明{\tt p1}和{\tt p2}不是同一個對象, 這符合我們的預期. 
對於{\tt ==}, 本來我們看兩者包含數據一樣, 預期會是{\tt True}. 
但在此處, 令你沮喪的是, 對於實例來說, {\tt ==}運算符的默認操作和
{\tt is}運算符是一樣的; 都是檢驗對象是否相同, 而不是判斷是否相等.
...
圖~\ref{fig.rectangle2}展示了上面程序的對象圖.
\index{state diagram}
\index{diagram!state}
\index{object diagram}
\index{diagram!object}
...
雖然可行, 但不夠高效. 
做個練習, 編寫一個不包含任何循環的{\tt increment}正確版本. 

修改器所為, 純函數皆可為. 
事實上, 有些編程語言只允許使用純函數. 
有證據表明, 使用純函數比使用修改器, 開發更快, 出錯更少. 
但有時修改器用來更方便, 而函數式程序則效率不高. 
...
和 \verb"int_to_time"), 便可以得到一個更精簡, 易讀, 易調試, 也更可靠的程序. 
...
\item[原型和補丁(prototype and patch):] 一種通過先寫程序初稿, 然後測試, 並修正發現的錯誤的開發方案. 
\index{prototype and patch}
...
\item[函數式編程風格(functional programming style):] 一種程序設計風格, 其中多數函數
都是純函數.
\index{functional programming style}

\item[不變式(invariant):] 程序執行期間, 應該始終為真的條件. 
\index{invariant}
...
\item 使用 {\tt datetime} 模塊編寫程序, 獲取當前日期並打印星期幾. 

\item 編寫個程序, 輸入生日, 輸出用戶的年齡以及距離下個生日的
天數, 小時數, 分鐘數和秒數. 
\index{birthday}
...
這些功能嚴格來說並非必須;
多數功能的語法我們都已經變相實現過了. 
但很多時候, 面向對象的語法更加簡潔, 同時更加精確地表達程序結構. 
...
通過定義一些特殊方法, 你可以對自定義類型, 重新定義運算符的行為. 
比如, 你為{\tt Time}類定義了\verb"__add__" 方法, 便可以用{\tt +}運算符操作Time對象. 
\index{programmer-defined type}
\index{type!programmer-defined}
...
\end{verbatim}
%
當對Time對象應用{\tt +}運算符時, Python會調用\verb"__add__".
當打印結果時, Python會調用\verb"__str__".
所以我們看到的, 只是冰山一角而已！
\index{operator overloading}

對自定義類型, 改變運算符的行為, 這便是{\bf 運算符重載}. 
對於Python中的每個運算符, 都有一個類似\verb"__add__"的特殊方法與之對應. 
更多詳細信息, 請參考
...
\end{verbatim}
%
這個函數對於列表, 元組, 甚至字典都適用, 只要{\tt s}中的元素是可哈希的, 
就都可以作為{\tt d}的鍵. 
...
面向對象設計的一個目標便是令軟件便於維護, 
也就是說當系統的某些部分發生了變化, 你依然可以保證程序正常運行, 
從而可以修改程序, 以滿足新的需求. 
\index{interface}
\index{implementation}
\index{maintainable}
...
但如果程序的其他部分正在使用你的類, 這時候修改接口, 
往往比較耗時, 且容易出錯. 

但如果你設計接口時足夠仔細, 你便可以在不改變接口的同時, 修改內部實現, 
也就意味著程序其他部分無需變動. 
...
\item[運算符重載(operator overloading):] 修改類似{\tt +}這樣運算符的操作, 
從而令其適用於自定義類型. 
\index{overloading}
\index{operator!overloading}
...
我用\mymapsto~符號表示映射比較清晰明了, 但不能寫入Python程序中. 
它們只是程序設計的一部分, 不應出現在代碼中. 
\index{Card class}
\index{class!Card}
...
\verb"Card.rank_names"表示與類相關的字符串列表. 
...
從{\tt Card}類的\verb"rank_names"列表中, 獲取相應的字符串.''

\verb"rank_names"的第一個元素是{\tt None},
因為沒有卡牌的等級為零. 
通過囊括{\tt None}作為一個占位符, 
從而令索引2恰到好處地映射到了字符串\verb"'2'", 其他一樣. 
如果不想要這種取巧操作, 可以用字典替代列表. 
...
既然整副牌是由卡牌構成, 那自然每副牌都應該包含一個卡牌列表作為屬性. 
\index{init method}
\index{method!init}
...
\end{verbatim}
%
這個方法展示了一個拼接超大字符串的高效方法: 建立一個字符串列表, 然後使用字符串方法{\tt join}進行拼接. 
...
\end{verbatim}
%
由於{\tt pop}是從列表中移除{\em 最後一張}牌, 所以我們從底部發牌. 
\index{append method}
\index{method!append}

若要添加一張牌, 可以用列表方法{\tt append}:
...
這裡, \verb"add_card" 是一個``薄''方法, 用卡牌術語的方式, 表達列表操作. 
而這會令實現的外觀, 或者接口, 容易理解. 
...
做個練習, 用列表方法{\tt sort}為Deck編寫一個{\tt sort}方法, 
為{\tt Deck}中的卡牌排序. 
...
但沒有滿足我們的需求：init方法本應用空列表初始化{\tt cards}, 
而不是用52張新牌. 
...
我們已經接觸過表示程序狀態的棧圖, 以及展示對象的屬性以及值的對象圖. 
這些圖好比程序運行時的快照, 因此也會隨著程序運行而變化. 
...
一個更詳細的類圖, 可能會顯示Deck實際上包含了一個Card{\em 列表}, 
但是一般類圖中不包含列表和字典這些內置類型. 
...
\verb"find_defining_class"函數使用{\tt mro}方法, 獲取類對象的列表, 
進而根據方法進行搜索. 
...
這種情況下, 你需要一個不同的開發模式. 
之前我們通過封裝和泛化來設計函數接口, 同樣我們也可以通過{\bf 數據封裝}
來設計類接口. 
...
如果我們讀取兩個文本, 他們的前置和後置詞彙都會被添加到同一個數據結構
中(會生成一些有趣的文本). 
...
\item 一旦程序可以運行, 便可尋找全局變量和使用它們的函數之間的關聯.
...
\item[偽裝方法(veneer):] 一個方法或者函數, 無需太多計算便可以為其他方法提供不同的接口. 
\index{veneer}
...
\item[類圖(class diagram):] 表示程序中各個類及其之間關係的圖解. 
\index{class diagram}
\index{diagram!class}
...
此方法會創建相應數量的手牌對象, 給每幅手牌分配相應數量的卡牌, 然後返回一個手牌實例的列表. 
\end{exercise}
...
為了避免程序異常退出, 我們使用一個``NaN'', 這個符號是一個特殊的浮點數, 
表示``不是一個數字''. 
\index{NaN}
\index{floating-point}
...
比如, 下面函數接收一個字符串列表, 將字符串方法{\tt capitalize}映射到每個元素上, 
並返回一個新的字符串列表:
...
方括號操作符表示我們在構造一個新列表. 
方括號中的表達式, 指定了列表中的元素, 同時{\tt for}語句聲明了我們要遍歷的序列. 
\index{list}
\index{for loop}

列表推導式的語法略顯奇怪, 主要因為這個循環變量, 如例子中的{\tt s}, 在定義之前, 
便出現在了表達式中. 
\index{loop variable}

列表推導式也可以用來過濾. 
比如, 下面函數僅選擇{\tt t}中大寫字母的元素, 並返回新列表:
\index{filter pattern}
\index{pattern!filter}
...
列表推導式更加簡潔, 也更易讀, 至少對於簡單表達式是這樣的. 
而且, 它們相比同樣的for循環, 要更快, 甚至有時快很多. 
所以如果你因為我沒有更早提及而生氣, 我理解. 

但是, 我要申辯一下, 列表推導式通常更難調試, 
因為你不能在循環內部放置一個打印語句. 
...
{\bf 生成器表達式}和列表推導式相似, 但它使用圓括號而不是方括號:
...
但是與列表推導式不同, 它不會立即計算所有值; 而是在被調用時才計算. 
...
如果任意一個值為{\tt True}, 則返回{\tt True}. 通常適用於列表:
\index{any}
...
在第~\ref{dictsub}節, 我用字典來尋找出現在文檔中, 但是不在單詞列表中的單詞. 
該函數接收兩個參數, 包含文檔中單詞的參數{\tt d1}(作為鍵),  和包含單詞列表的參數{\tt d2}. 
...
比如, 下面是一個使用循環的\verb"uses_only"實現版本:
...
運算符\verb"<=" 會檢驗一個集合是否是另一個的子集, 包括兩者相等的情況, 
...
計數器提供了一些類似集合操作的方法和運算符, 
包括方法, 減法, 併集和交集. 
此外, 還提供了一個常用的方法, \verb"most_common", 這個方法
會返回一個按照最常見到最不常見排序的值-頻率對列表:
...
如果你要創建一個值為列表的字典, 可以使用{\tt defaultdict}來簡化代碼. 
在習題~\ref{anagrams}的答案(可以從\url{http://thinkpython2.com/code/anagram_sets.py}獲取)中, 
我創建了一個字典, 該字典將一個有序字母的字符串映射到了一個由這些字母構成的單詞的列表. 
...
但此方案也略有不足, 其不論是否必要, 每次都會創建新列表. 
對於列表來說, 尚無關緊要, 但如果工廠函數很複雜, 
那影響就很大了. 
\index{factory function}
...
這個方案也有缺陷, 不論是否需要, 每次循環都會創建{\tt Hand}對象. 
...
\item[列表推導式(list comprehension):] 一種方括號內包含{\tt for}循環, 產生新列表的表達式.
\index{list comprehension}

\item[生成器表達式(generator expression):] 一種圓括號內包含{\tt for}循環, 
從而產生生成器對象的表達式.  
\index{generator expression}
\index{expression!generator}
...
你可以使用緩存模式(參見第~\ref{memoize}節)來提升其效率. 
但是你會發現, 一旦使用了條件表達式, 緩存實現便變得困難. 
\end{exercise}
...
未終止的字符串可能會在程序末尾引發{\tt invalid token}異常, 
或者可能會將程序餘下部分當作字符串, 直至碰到下個字符串的標識. 
而在第二種情形下, 可能不會產生錯誤信息!
\index{multiline string}
\index{string!multiline}

\item 未封閉的開放運算符---\verb+(+, \verb+{+, 或
...
檢查編碼環境, 確保你編輯的便是Python運行的程序. 

如果你不確定, 則可以在程序開始的地方, 放個明顯且可控的異常. 
...
如果依然囿於困境, 不知緣由, 
一種方法是從一個類似``Hello, World!''的新程序開始, 重新構建, 
並確保該程序可以正常運行. 
然後將原來的程序代碼, 逐步遷移到新的程序中. 


\section{運行時異常}

如果你的程序語法都正常, Python便可以讀取並至少可以運行了. 
那可能會出現什麼問題呢?


\subsection{我的程序什麼也不做.}

這種情況, 往往由於你的文件雖然有函數和類, 但卻沒有調用函數來執行程序. 
這也可能是你有意為之, 因為你只是需要導入模塊, 提供類和函數而已. 

如果不是有意而為, 那便要確保程序中有調用函數的地方, 同時保證執行流程能
觸及函數調用(參見下面的``執行流程''). 
...
如果一個程序一直不運行, 看起來似乎什麼也沒做, 那便是程序``掛起''了. 
通常是指程序陷入了無限循環或者無盡遞歸中. 

\begin{itemize}

\item 如果你對程序中某個循環有所疑慮, 可以在循環開始前添加一個{\tt print}語句, 
打印``進入循環'', 在循環結束的地方, 添加一個打印``退出循環''的語句. 

運行程序, 如果你只得到第一個信息, 而第二個沒有出現, 
那便是陷入了無限循環. 可以跳轉到下面的``無限循環''章節, 深入理解. 

\item 多數情況下, 無盡遞歸會令程序運行運行一段時間後, 產生``RuntimeError: Maximum
...
\item 如果上面步驟都無效, 那便需要檢查其他循環以及遞歸的函數和方法了. 

\item 如果仍然無效, 那可能是你對於程序中的執行流程不太清楚. 
可以參看後面的``執行流程''章節. 
\end{itemize}
...
如果你認為程序中存在無限循環, 同時知道出問題的循環所在, 那便可以在
循環的末尾添加{\tt print}語句, 輸出條件中變量的值以及條件的結果. 
...
\end{verbatim}
%
現在運行程序, 你會看到每次循環都有三行輸出. 
最後一次循環時, 條件應該是{\tt False}.
如果循環繼續執行, 那你便可以看到{\tt x}和{\tt y}的值, 
同時你便會發現其值未被正確更新的原因.
...
現在運行程序, 隨著函數調用, 會輸出每個函數的軌跡. 


\subsection{運行程序, 得到異常.}
\index{exception}
\index{runtime error}
...
返回{\tt None}. 
另外一個常見的原因是, 直接使用了某些列表方法的結果, 比如{\tt sort}, 
也會返回{\tt None}.
\index{AttributeError}
\index{exception!AttributeError}

\item[索引異常(IndexError):] 用來訪問列表, 字符串或元組的索引, 超過了其長度減一. 
在錯誤發生位置之前, 添加{\tt print}語句, 顯示索引的值和數組長度. 
看看數組長度是否正確? 索引是否正確?
\index{IndexError}
\index{exception!IndexError}
...
使用{\tt print}語句調試程序的一大難題在於, 你將被大量輸出所淹沒. 
有兩種解決途徑：簡化輸出, 或簡化程序. 
...
若要簡化程序, 可以從以下幾個方面著手. 
首先, 縮減程序所解決問題的規模. 
比如, 如果你要檢索列表, 用{\tt 小的}列表. 
如果程序是從用戶獲取輸入, 那便提供一個引發問題的最簡輸入. 
\index{dead code}

第二, 清理程序. 移除無用代碼, 重新組織程序, 使其便於理解. 
例如, 如果你懷疑問題出在程序的深層嵌套中, 
嘗試用簡單結構優化這部分. 
...
如果你發現程序在某個條件下運行正常, 但是另外條件下, 不正常, 
這就給了你解決所面臨問題的線索. 

同樣, 重寫代碼可以幫你發現一些微妙的錯誤. 
如果你改變了一些代碼, 以為不會影響程序, 但現實是影響到了, 
這也在某方面能給你一些提示. 
...
第一步是在程序代碼和你之所見之間, 建立連接. 
你需要先假設程序做了某些事情. 
令調試如此困難的一方面, 在於計算機運行太快了. 

你會期望可以減慢程序運行速度, 從而滿足人類的速度, 
確實有些調試器可以滿足你. 
但相比較設置調試器, 插入或移除斷點, 以及``步進''程序到錯誤出現的位置, 其所耗費時間, 
...
編程之前, 你需要先構建程序運行的邏輯模型. 
如果你寫的程序運行不符合預期, 
通常問題不在代碼上; 而在於你腦海中的模型. 
\index{model, mental}
\index{mental model}

修正邏輯模型的最好方式, 是將程序拆解為組件(通常是函數和方法), 同時
單獨測試每個部件. 
...
不僅能確保程序正確(按期望順序執行), 同時對於未能記憶運算符優先級的人來說, 
也便於閱讀. 
...
\item 隨機遊走編程(企圖編寫所有可能的程序, 從中選擇正確執行的那個).
\index{random walk programming}
\index{development plan!random walk programming}
...
問題時有發生. 即使最好的程序員, 偶爾也會陷入困境. 
有時候你專注在程序上的時間太長了, 以致於無法發現錯誤. 
你需要的是一雙重新審視的眼睛. 

在你向他人尋求幫助之前, 要確保你做好了準備, 
你的程序要盡量簡單, 並且程序在盡量少的輸入下, 可以產生錯誤. 
...
要記住, 目標不僅僅是讓程序正常工作, 更是要學會如何令程序正常工作. 
...
{\bf 算法分析}是計算機科學的一個分支, 主要研究算法的性能, 特別是其運行時間和所需空間. 
請參閱
...
其性能對於大部分應用來說已經足夠了, 但如果我的應用程序確實太慢, 
我會用性能分析器來查看時間耗費在哪裡. 
...
索引算法--讀寫序列或字典中的元素--是常數時間, 無論數據結構大小如何. 
\index{indexing}

遍歷序列或字典的{\tt for}循環通常是線性時間, 前提是
循環體中的所有操作都是常數時間. 
例如, 對列表中元素進行累加便是線性耗時:
...
經驗來說, 如果循環體內的複雜度是$O(n^a)$, 那麼整個循環的複雜度便是
$O(n^{a+1})$. 
例外情況在於, 你確定循環會在特定疊代次數後退出. 
如果不管$n$是多大, 循環僅運行$k$次, 
那麼即使$k$很大, 該循環的複雜度也是$O(n^a)$.

乘以$k$不會改變增長量級, 同樣除以一個數也不會. 
所以如果循環體複雜度為 $O(n^a)$, 並且運行$n/k$次, 無論$k$多大, 
循環的複雜度都是$O(n^{a+1})$. 
...
大部分列表方法的耗時都是線性的, 但也有一些例外情況:
\index{list methods}

\begin{itemize}

\item 平均情況下, 向列表末尾增加一個元素是常數時間;
當用盡空間時, 偶爾需要複製到更大空間, 
...
但如果你遍歷疊代器, 那這個循環便是線性時間. 
\index{iterator}

\end{itemize}

字典的出現是計算機科學的一個小奇蹟. 在第~\ref{hashtable}節我們會
看到它們是如何工作的. 
...
二分檢索和你從字典(紙質版詞典, 不是數據結構)中查找單詞的方法很相似. 
...
還有一種數據類型, 叫做{\bf 哈希表}, 檢索更快---它可以
在常數時間內進行檢索---同時不需要對項目排序. 
Python字典便是使用哈希表實現, 這也是為什麼大量字典操作, 包括{\tt in}操作, 
都是常數時間的原因. 
...
{\tt add}會將一個鍵-值元組加入數據項列表, 這需要常數時間.

{\tt get}使用一個{\tt for}循環來檢索列表:
如果找到目標鍵, 則返回對應的值;
否則, 拋出{\tt KeyError}.
所以{\tt get}耗時是線性時間.
...
但是在列表中插入新項的耗時變成了線性的,
所以這可能不是最佳選擇.
也有其他數據結構可以用對數時間實現{\tt add}和{\tt get}操作,
但是仍然不如常數時間好, 那麼讓我們繼續.
\index{red-black tree}

一種優化{\tt LinearMap}的方法是將鍵-值對列表拆分並放入更小的列表中. 
有一種實現方式叫做{\tt BetterMap}, 這是一個包含100個LinearMap的列表.
稍後我們會看到, 雖然其{\tt get}的時間增長量級仍然是線性時間,
但是{\tt BetterMap}是邁向哈希表的重要一步:
...
\verb"__init__"創建了一個包含{\tt n}個{\tt LinearMap}的列表.
...
並返回一個整數.
但局限之處在於, 只對可哈希化的鍵有效. 對列表和字典這種可變類型, 因其不可哈希化而無效.
\index{hash function}
...
若要了解其原理, 想像從一個空哈希表開始, 向其中添加一系列的項.
...
這個算法的一個重要特性是, 當我們調整哈希表大小時, 其耗時呈幾何方式增長;
也就是說, 其大小乘以一個常數.
如果你增大其規模---每次增加固定數量---每次{\tt add}的平均耗時便是線性的.
\index{geometric resizing}

你可以從\url{http://thinkpython2.com/code/Map.py}下載我的哈希表的實現代碼,
...
\item[檢索(search):] 定位集合(列表或字典)中的元素或確認其不存在的過程.
\index{search}

\item[哈希表(hashtable):] 一種數據結構, 用來表示鍵-值對的集合, 同時
可以用常數時間進行檢索.
\index{hashtable}
