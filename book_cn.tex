...
1999年1月, 我正准备以Java教学生编程入门, 
课程已讲过三次, 但我沮丧依旧. 
因为多人无法完成目标, 虽有优秀个体, 但整体效果令人难以接受.

依我观察, 问题出在书本教案上, 
书籍厚重, 画蛇添足, 细枝末节过多, 编程思虑不足.
...
首版略显粗略, 但是简单有效. 
学生知其然, 并知其所以然, 我便可聚焦课堂于难点, 趣题和操作实践. 
绝知此事要躬行, 实践是艰难且不可避免的, 却也是最有趣的. 
...
2003年, 我开始在欧林学院任职, 并且首次教授Python. 
和教授Java相比, 惊喜非凡, 学生乐于学习且乐在其中. 
\index{Olin College}

自此, 我不断优化此书, 校正错误, 提高案例水准, 
并不断增加一些新的例子和练习. 

因此本书有个通俗易懂的名字{\em Think Python}. 
部分修改如下:
...
\item 对于第~\ref{turtle}节, 我将本人常用的海龟绘图包 Swampy 换成了功能更丰富, 
应用更广泛的 {\tt turtle}包. 

\item 增加了新章节 "利器", 主要介绍一些 Python 不常用但是高效的功能. 
...
感谢整理{\em How to Think Like a Computer Scientist}的Lulu的编纂. 

感谢耐心编辑 {\em Think Python}的O'Reilly Media的朋友. 

感谢使用本书早期版本的学生们, 感谢所有提出建议和提出错误的朋友. 
...
以及Jonah Cohen 的生成HTML的Prel脚本中的一个错误. 
...
\item Eric Bronner 指出了关于执行顺序的讨论中的一处歧义. 
...
比如数学, 计算机专家用专业语言去表达观点(尤其涉及计算). 
比如工程师, 他们设计原件, 拼装组件, 构建系统, 权衡各种方案. 
...
\item[循环执行:] 在宿命轮回中不断尝试, 于既定界限处终达彼岸. 
...
括号表示{\tt print}是个函数, 后续在第~\ref{funcchap}节会介绍. 
...
表示位运算的 "异或" 操作. 如果不了解位运算, 那下面的例子只会让你困惑. 
...
通常是人类自然进化而得来, 并不是出于特定目的而创造. 


{\bf 形式语言} 指人们为了特定目的而创造的特定格式的语言, 
比如数学家们发明的数学符号, 
可以很方便表示数字和符号的关系. 
而化学家发明的分子结构式也是一种形式语言, 更为重要的是：

\begin{quote}
{\bf 人们为了表示机器运算而创造的形式语言就是编程语言. }
\end{quote}

形式语言一般有严格的{\bf 语法} 来管理语句的结构. 例如, 
数学表示, $3 + 3 = 6$ 是正确的, 而 $3 + = 3 \$ 6$ 则不正确. 
又比如在化学中, $H_2O$ 正确, 而 $_2Zz$ 则明显有问题. 
\index{syntax}

语法包括两部分, {\bf 符号} 和 规则. 
符号是语言的基本, 比如单词, 数字和化学元素等. 
$3 += 3 \$ 6$的问题在于, \(\$\) 在数学中不是一个合法的符号(据我所知). 
同样, $_2Zz$ 在化学领域不正确, 也是因为$Zz$ 无法对应相应的元素. 
\index{token}
\index{structure}

语法的另外一个要素是{\bf 规则}, 即把符号有效组织起来所遵循的结构规范. 
公式$3 +/ 3$ 不合语法, 不是因为$+$ 和$/$ 不正确, 而是因为先后顺序问题. 
同样, 上述化学公式不仅仅是词汇胡编乱造, 其规则也有问题, 元素下标不能在元素之前, 
而应放在后边. 

This is @ well-structured Engli\$h
sentence with invalid t*kens in it. 这句话每个字母(符号)都正确, 
但是拼在一起, 就不正确了. 
...
形式语言和自然语言在符号, 规则以及语法等方面较为相似, 但是也有
很多不同之处：
\index{ambiguity}
\index{redundancy}
\index{literalness}
...
和相关情境确定对方的意思. 但是形式语言中, 语法确定, 语意清晰, 
不会出现同一句话表示不同意思的情况. 

\item[冗余:] 为了让对方明白自己的真实意思, 避免误会, 
自然语言中往往需要添加更多相关信息, 来辅助沟通. 
而形式语言则更加简炼且精确. 

\item[比喻:] 自然语言中充满了俗语和比喻, 我说 "The penny dropped", 
不是说谁得钱掉了(比喻恍然大悟). 形式语言则没有类似的表述. 
...
形式语言相比自然语言, 更加细致繁密, 所以需要花更多时间
阅读理解. 其次, 语法结构也很重要, 所以不要总是从上往下, 从左往右
地阅读, 而要在大脑中分析代码, 识别特有符号, 解读语法规则. 
最后, 要格外关注细节, 单词错误, 标点缺失, 这些问题在自然语言的使用中
可能影响不大, 但在形式语言中, 却是致命错误. 
...
\item[形式语言(formal language):] 人类因特定目标而创造的语言. 
例如数学语言, 编程语言; 所有的编程语言都是形式语言. 
\index{formal language}

\item[符号(token):] 程序中的基本元素, 类似自然语言中的单词. 
\index{token}

\item[语法(syntax):] 程序的结构规范. 
\index{syntax}

\item[解析(parse):] 分析语法结构并检验程序. 
\index{parse}
...
这种形象化展示变量所处状态(所对应的值)的图叫做 {\bf 状态图}, 
图~\ref{fig.state2} 便是上面代码示例的状态图. 
...
程序工程师一般会为变量起个容易理解记忆的名字, 使人一看即知其用途. 
...
\end{verbatim}
%
变量名 {\tt 76trombones}  的错误在于数字开头, 
而{\tt more@} 是因为包含了不合法字符 {\tt @}. 
那么 {\tt class} 为什么也不正确呢？

这是因为 {\tt class} 是 Python的一个 {\bf 关键字}. 
解释器用关键字(也称预留字)来识别解析代码, 所以关键字不能作为变量名使用. 
\index{keyword}
...
\item 乘法({\bf M}ultiplication)和除法({\bf D}ivision)优先于加法({\bf A}ddition)
和减法({\bf S}ubtraction). 所以 {\tt 2*3-1} 等于 5而非4, {\tt 6+4/2} 等于 8而非 5. 
...
但是, 数字的加法和乘法与字符串的拼接和堆叠, 在某方面的应用会有明显不同. 
你是否可以想到, 加法运算正常, 而字符串拼接却表现截然不同的场景？
...
随着代码量越来越庞大, 越来越复杂, 阅读并理解其逻辑也越来越困难. 
形式语言使用时又写得密密麻麻, 对着满页代码去明白其逻辑和功能, 便异常艰难. 

所以, 在程序代码中加入一些自然语言来进行解释, 说明其作用, 便显得异常重要. 
...
编码中实现特殊逻辑时, 注释显得尤为重要. 
要记住, 说明编码的 {\em 缘由} , 远胜于讲述代码的{\em 作用}. 
...
\item[语法错误:] "语法" 指代码结构以及使用规范. 
例如, 括号必须成对出现, 
{\tt (1 + 2)}正确, 而{\tt 8)}便存在语法错误. 
\index{syntax error} 
\index{error!syntax}
\index{error message}
\index{syntax} 

如果程序存在语法错误, Python会直接报错并退出执行, 
程序便无法继续运行. 
开始学习编程时, 需要耗费大量精力跟踪语法错误. 
随着经验增长, 此类错误会越来越少, 而定位也会越来越快. 
...
\item[求值(evaluate):]  执行运算产生值, 从而简化表达式的过程. 
\index{evaluate}

\item[语句(statement):]  表示命令或者操作的代码块. 目前遇到的语句包括赋值语句以及打印语句. 
\index{statement}

\item[执行(execute):]  通过执行计算来简化表达式, 从而产生值.
\index{execute}
...
\item 如果我6:52离家, 以慢跑方式 (8分15秒/每英里)跑了1英里, 然后以
